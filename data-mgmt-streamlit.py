...
def parse_sql_blocks(sql_text: str):
    ...
    # sort by num in case out of order
...
# If no blocks were parsed, fall back to generic names for q1..q11
fallback_blocks = [
...
with chart_cols_left:
    ...
    if not y_candidates:
        ...
        # if possible, prefer a 'Total_Revenue' or similar column by default
        default_y_idx = 0
...
if y_col:
    try:
        ...
        # Try to keep ordering sensible for categorical x
        if not is_numeric(data[x_col]):
            # Limit cardinality to avoid huge charts
            top_n = 50
            if len(data[x_col].unique()) > top_n:
                top_vals = data[x_col].value_counts().nlargest(top_n).index
                data = data[data[x_col].isin(top_vals)]

        enc = alt.Chart(data).mark_bar() if chart_type == "Bar" else (
            alt.Chart(data).mark_line() if chart_type == "Line" else (
                alt.Chart(data).mark_area() if chart_type == "Area" else alt.Chart(data).mark_circle(size=60)
            )
        )
        ...
        st.altair_chart(base.properties(height=380, width="container"), use_container_width=True)
